...
app.post('/bruxos', async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO bruxos (nome, idade, casa, habilidade, sangue, patrono) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [nome, idade, casa, habilidade, sangue, patrono]);
        res.status(201).send({
            status: "sucesso",
...
app.post('/varinhas', async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO varinhas (material, comprimento, nucleo, data_criacao) VALUES ($1, $2, $3, $4) RETURNING *', [material, comprimento, nucleo, data_criacao]);
        res.status(201).send({
            status: "sucesso",
...
app.put('/varinhas/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { material, comprimento, nucleo, data_criacao } = req.body;
        await pool.query('UPDATE varinhas SET material = $1, comprimento = $2, nucleo = $3, data_criacao = $4 WHERE id = $5', [material, comprimento, nucleo, data_criacao, id]);
        res.status(200).send({
            status: "sucesso",
